  ...
  const handleDelete = useCallback(
    (id: string, rule: string) => {
      confirm({
        ...
        message: rule
          ? `"${rule}"\n\nThis policy will stop applying to new decisions.`
          : "This policy will stop applying to new decisions.",
    ...
    <>
      ...
      >
        <div style={{ fontSize: 13, color: "var(--text-secondary)" }}>
          Office operating rules. Set explicitly or auto-detected from working
          patterns.
        </div>
        <button
      ...
      {!(isLoading || error) && activePolicies.length > 0 && (
        <div style={{ padding: "8px 0" }}>
          {SECTIONS.map((section) => {
            const sectionPolicies = activePolicies.filter(
              (p) => p.source === section.key,
            );
