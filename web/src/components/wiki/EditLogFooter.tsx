...
import { subscribeEditLog, MOCK_EDIT_LOG, type WikiEditLogEntry } from '../../api/wiki'
...
export default function EditLogFooter({ initialEntries, onNavigate }: EditLogFooterProps) {
      ...
      {entries.map((entry, idx) => {
        ...
        return (
          ...
          >
            ...
            <span className="wk-when">{isLive ? 'just now' : safeRelative(entry.timestamp)}</span>
          </span>
        )
      })}
    </div>
