...
describe('<WikiArticle>', () => {
  ...
  it('renders Sources populated from fetchHistory with author slugs visible', async () => {
    ...
    vi.spyOn(api, 'fetchHistory').mockResolvedValue({
      commits: [
        { sha: 'aaaaaaa1111', author_slug: 'ceo', msg: 'Initial brief', date: '2026-01-16T00:00:00Z' },
        { sha: 'bbbbbbb2222', author_slug: 'pm', msg: 'Add pilot scope', date: '2026-01-17T00:00:00Z' },
        { sha: 'ccccccc3333', author_slug: 'cro', msg: 'Pricing note', date: '2026-01-18T00:00:00Z' },
      ],
    })

    // Act
    render(<WikiArticle path="people/customer-x" catalog={CATALOG} onNavigate={() => {}} />)

    // Assert
    const sourcesHeading = await screen.findByRole('heading', { name: 'Sources' })
  ...
  it('renders nothing for Sources when fetchHistory rejects', async () => {
    // Arrange
    vi.spyOn(api, 'fetchArticle').mockResolvedValue(STUB_ARTICLE)
    vi.spyOn(api, 'fetchHistory').mockRejectedValue(new Error('git log unavailable'))
