 ...
 * with fallback to ~/.wuphf/config.json.
 */
 ...
 *
 * Priority: WUPHF_API_KEY env > ~/.wuphf/config.json
...
export function loadConfig(): NexConfig {
  ...
  let baseUrl = process.env.WUPHF_API_BASE_URL ?? loadFileConfig().dev_url ?? "https://app.nex.ai";
