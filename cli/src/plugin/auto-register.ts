 ...
 *
 * On success: prints API key and saves to ~/.wuphf/config.json.
...
async function main(): Promise<void> {
  ...
  // Check if already registered (checks ~/.wuphf/config.json then legacy ~/.wuphf-mcp.json)
  try {
    const existing = loadConfig();
    console.log("Already registered.");
    console.log(`  API key: ${existing.apiKey.slice(0, 12)}...`);
    console.log(`  Config: ${MCP_CONFIG_PATH}`);
    console.log("\nTo re-register, delete ~/.wuphf/config.json first.");
    return;
  } catch (err) {
