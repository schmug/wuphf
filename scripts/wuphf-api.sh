#!/usr/bin/env bash
# WUPHF API wrapper — safe HTTP client for OpenClaw skills
...
BASE_URL="${WUPHF_DEV_URL:-https://app.nex.ai}/api/developers"
TIMEOUT=120

# --- Validate environment ---
if [[ -z "${WUPHF_API_KEY:-}" ]]; then
  echo "Error: WUPHF_API_KEY environment variable is not set" >&2
  exit 1
fi
...
if [[ -z "$METHOD" ]]; then
  echo "Usage: wuphf-api.sh <METHOD|sse> <path>" >&2
...
if [[ "$METHOD" == "sse" ]]; then
  exec curl -N -s --max-time "$TIMEOUT" \
    -H "Authorization: Bearer ${WUPHF_API_KEY}" \
